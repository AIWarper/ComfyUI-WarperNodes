...
def draw_keypoints_and_connections(canvas, keypoints_array, connections, color_map_or_default_color, thickness, point_radius, is_body=False):
    ...
    if isinstance(connections, list) and connections:
        ...
        elif isinstance(connections[0], tuple) and len(connections[0]) == 2 and isinstance(connections[0][0], tuple):
             for (p1_idx, p2_idx), color_val in connections:
                pt1 = get_point(keypoints_array, p1_idx)
                pt2 = get_point(keypoints_array, p2_idx)
                if pt1 is not None and pt2 is not None:
    ...
    for i in range(keypoints_array.shape[0]):
        ...
        if pt is not None:
            point_color_to_use = WHITE
            if is_body and isinstance(color_map_or_default_color, list):
                found_color = False
                for (p1_idx_conn, p2_idx_conn), color_val_conn in color_map_or_default_color: # color_map_or_default_color is BODY_DRAW_CONFIG
...
def draw_scaled_pose(people_data, canvas_height, canvas_width):
    ...
    face_hand_point_rad = max(1, int(body_point_rad / FACE_HAND_POINT_RADIUS_SCALE_FACTOR))
...
def get_point_raw_coords(keypoints_array, index):
    if keypoints_array.ndim == 2 and 0 <= index < keypoints_array.shape[0] and keypoints_array[index, 2] > 0.01:
        return keypoints_array[index, :2].astype(float)
    return None
...
def set_point(keypoints_array, index, new_coords_np_array, confidence_override=None):
    if keypoints_array.ndim == 2 and 0 <= index < keypoints_array.shape[0]:
        keypoints_array[index, :2] = new_coords_np_array
...
class DWPoseScalerNode_Warper:
    ...
    RETURN_TYPES = ("IMAGE", "POSE_KEYPOINT"); RETURN_NAMES = ("scaled_image", "scaled_pose_keypoints"); FUNCTION = "scale_pose"; CATEGORY = "Warper Tools"
    ...
    def scale_pose(self, pose_keypoints, neck_length_scale, torso_adjust_y, head_scale, arm_length_scale, hand_scale, leg_length_scale, keep_feet_grounded, reference_frame_index):
        if not pose_keypoints or not isinstance(pose_keypoints, list) or not pose_keypoints[0] or not isinstance(pose_keypoints[0], dict) or "canvas_height" not in pose_keypoints[0]:
            ...
            dummy_img = torch.zeros((1, 256, 256, 3), dtype=torch.float32); return (dummy_img, [])
        modified_frames_data = []; output_images_np_list = []
        ...
        for frame_idx, frame_data_orig_outer in enumerate(pose_keypoints):
            ...
            canvas_height = frame_data.get("canvas_height", 512); canvas_width = frame_data.get("canvas_width", 512)
            if "people" not in frame_data or not frame_data["people"]:
                img_np = np.zeros((canvas_height, canvas_width, 3), dtype=np.uint8); output_images_np_list.append(img_np); modified_frames_data.append(frame_data); continue
            ...
            for person_idx, person in enumerate(frame_data["people"]):
                ...
                if keep_feet_grounded and leg_length_scale != 1.0:
                    y_offset_total = 0.0; num_ankles_for_offset = 0
                    ...
                    if target_l_ankle_y is not None and new_l_ankle_coords is not None: y_offset_total += (target_l_ankle_y - new_l_ankle_coords[1]); num_ankles_for_offset +=1
                    new_r_ankle_coords = get_point_raw_coords(body_kp, R_ANKLE)
                    if target_r_ankle_y is not None and new_r_ankle_coords is not None: y_offset_total += (target_r_ankle_y - new_r_ankle_coords[1]); num_ankles_for_offset +=1
                    if num_ankles_for_offset > 0:
                        ...
                        for kp_array_curr in [body_kp, face_kp, hand_l_kp, hand_r_kp]:
                            ...
                            for i in range(kp_array_curr.shape[0]):
                                if kp_array_curr[i,2] > 0.01: kp_array_curr[i, 1] += final_y_offset
                
                # --- 3. Neck Length Scaling (Option C revised: Upper Neck Stretch without intrinsic head scaling) ---
                if neck_length_scale != 1.0:
                    print(f"DEBUG: --- Upper Neck Stretch (Option C rev.) START: Frame {frame_idx}, Person {person_idx}, Scale: {neck_length_scale} ---")
                    
                    # The NECK keypoint (index 1) is the primary anchor relative to the body.
                    # Its position is determined by previous steps and is NOT modified by this specific scaling block.
                    current_neck_coords_anchor_body = get_point_raw_coords(body_kp, NECK)
                    
                    if current_neck_coords_anchor_body is None:
                        print(f"DEBUG:   Skipping Upper Neck Stretch: NECK keypoint (body anchor) is missing.")
                    else:
                        print(f"DEBUG:   Body Anchor NECK position (not moved by this step): {current_neck_coords_anchor_body}")

                        # The NOSE will act as the "head anchor" that we move relative to the NECK.
                        # Other head parts will move rigidly with the NOSE.
                        original_nose_coords = get_point_raw_coords(body_kp, NOSE)

                        if original_nose_coords is None:
                            print(f"DEBUG:   Skipping Upper Neck Stretch: NOSE keypoint (head anchor) is missing.")
                        else:
                            print(f"DEBUG:     Original NOSE (head anchor) position: {original_nose_coords}")

                            # Calculate the original vector from the body's NECK anchor to the NOSE (head anchor)
                            orig_vec_neck_to_nose = original_nose_coords - current_neck_coords_anchor_body
                            print(f"DEBUG:       Original vector NECK_body_anchor to NOSE_head_anchor: {orig_vec_neck_to_nose}")

                            # Scale this vector
                            scaled_vec_neck_to_nose = orig_vec_neck_to_nose * neck_length_scale
                            print(f"DEBUG:       Scaled vector NECK_body_anchor to NOSE_head_anchor: {scaled_vec_neck_to_nose}")

                            # Calculate the new position for the NOSE (head anchor)
                            new_nose_coords = current_neck_coords_anchor_body + scaled_vec_neck_to_nose
                            print(f"DEBUG:     New NOSE (head anchor) target position: {new_nose_coords}")
                            
                            # Store original vectors from the OLD NOSE to other head dependents
                            # These vectors define the rigid structure of the head relative to the NOSE.
                            head_body_dependents_indices_rel_to_nose = [R_EYE, L_EYE, R_EAR, L_EAR] # Excludes NOSE itself
                            ...
                            # First, set the new position for the NOSE (head anchor)
                            set_point(body_kp, NOSE, new_nose_coords)
                            # For re-attachment, use the actual new NOSE position after set_point (accounts for potential clipping/rounding)
                            current_new_nose_coords_after_set = get_point_raw_coords(body_kp, NOSE) 
                            print(f"DEBUG:     NOSE (head anchor) actually set to: {current_new_nose_coords_after_set}")


                            # Now, re-attach other head dependents to the NEW NOSE position, maintaining their original relative positions to the NOSE
                            if current_new_nose_coords_after_set is not None: # Check if NOSE was successfully set
                                for kp_idx, orig_vec_from_nose in orig_vectors_from_orig_nose_to_body_dependents.items():
                                    if orig_vec_from_nose is not None:
                                        ...
                                        # if kp_idx == L_EYE: print(f"DEBUG:       L_EYE re-attached to: {get_point_raw_coords(body_kp, L_EYE)}")
                                ...
                                print(f"DEBUG:     Skipped re-attaching head dependents as new NOSE position is None (likely an issue with set_point or NOSE missing after all).")
                    
                    print(f"DEBUG: --- Upper Neck Stretch (Option C rev.) END: Frame {frame_idx}, Person {person_idx} ---")
                ...
                if torso_adjust_y != 0.0:
                    ...
                    body_kp_orig_state = body_kp.copy() # State after all scaling, before torso shift
                    face_kp_orig_state = face_kp.copy()
                    hand_l_kp_orig_state = hand_l_kp.copy()
                    hand_r_kp_orig_state = hand_r_kp.copy()

                    # Anchor points for torso shift (Neck, Shoulders)
                    # These are their positions *before* this y_offset is applied
                    ...
                    # Move the primary torso anchors
                    ...
                    # Move dependents relative to their respective new anchor positions
                    # Head parts move with the NECK
                    if orig_neck_for_torso_adjust is not None and new_neck_pos_torso_adj is not None:
                        for kp_idx in [NOSE, R_EYE, L_EYE, R_EAR, L_EAR]: # Head parts on body_kp
                        ...
                        if face_kp_orig_state.size > 0: # All face_kp points
                    ...
                    # Left arm parts move with L_SHOULDER
                    ...
                    # Right arm parts move with R_SHOULDER
                    ...
                    # Hands move with their respective WRISTS (which would have moved with shoulders)
                    # Need to get wrist positions *after* they've been moved by shoulder y-adjust
                    orig_l_wrist_for_torso_adjust = get_point_raw_coords(body_kp_orig_state, L_WRIST) # Wrist pos BEFORE y-adjust
                    new_l_wrist_pos_torso_adj = get_point_raw_coords(body_kp, L_WRIST) # Wrist pos AFTER y-adjust
                    ...
                    orig_r_wrist_for_torso_adjust = get_point_raw_coords(body_kp_orig_state, R_WRIST) # Wrist pos BEFORE y-adjust
                    new_r_wrist_pos_torso_adj = get_point_raw_coords(body_kp, R_WRIST) # Wrist pos AFTER y-adjust
                ...
                a_min_kps = np.array([0, 0, 0], dtype=float); a_max_kps = np.array([canvas_width - 1, canvas_height - 1, 1.0], dtype=float)
